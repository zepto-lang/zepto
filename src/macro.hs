  ...
  if macroElementMatchesMany transform
     then do
             ...
             case lst of
                  ...
                  Nil _ -> return lst
                  _ -> throwError $ BadSpecialForm "transformRule: Macro transform error" $ List [(List ellipsisList), lst, (List [dl]), Number $ NumI $ toInteger ellipsisIndex]
...
transformRule _ ellipsisIndex result transform unused = do
  throwError $ BadSpecialForm "An error occurred during macro transform" $ List [(Number $ NumI $ toInteger ellipsisIndex), result, transform, unused]
